...
export const css = async (source, destination, env) => {
  const pre = await sass.compileAsync([source.dir,source.base].join('/'), {
  ...
  console.log(chalk.hex('#C24C89').bold(`sass ${[source.dir,source.base].join('/')}`))
  ...
  console.log(chalk.hex('#0e73d3').bold(`postcss ${[source.dir,source.base].join('/')}`))
...
export const js = async (source, destination, env) => {
  ...
  console.log(chalk.hex('#fec70b').bold(`esbuild ${`${destination}/${source.name}.js`}`))
}

...
(function ($, drupalSettings) {
  ...
  Drupal.behaviors.megaMenuInit = {
    attach: function attach() {
        ...
        if (!$(this).hasClass('tabbed')) {
          ...
          if ($('.js-mega-menu-target > .menu > li[data-id="' + menuItemClass + '"]').children('.menu').length > 0) {
            ...
            if (showMegaMenu) {
              ...
              var megaMenuTarget = $('.js-mega-menu-target');
      ...
      $('.js-mega-menu-target .menu .menu > .expanded').each(function () {
        var self = $(_this);
        var injectPos = $('> .menu > .menu__item:nth-child(' + (noLinks + 1) + ')', self);

        if (injectPos.length) {
          var li = $('<li />').insertBefore(injectPos);
